DROP TABLE IF EXISTS mp_estrella_db.dbo.fact_licitacion;
...
CREATE TABLE mp_estrella_db.dbo.fact_licitacion (
    ...
    cantidad_adjudicada  int not null,
    monto_adjudicado_clp numeric(36, 0) not null,
    monto_adjudicado_usd numeric(36, 0) not null,
...
WITH Temporal AS (
    SELECT DISTINCT
       l.fechaAdjudicacion as id_fecha,
       r.id_rubro,
       l.codigoProductoONU as id_producto,
       CASE ComunaUnidad
				...
				ELSE
                    CASE
                        WHEN RegionUnidad like '%Tarapaca%' THEN 1000
                        WHEN RegionUnidad like '%Tarapacá%' THEN 1000
                        WHEN RegionUnidad like '%Antofagasta%' THEN 2000
                        WHEN RegionUnidad like '%Atacama%' THEN 3000
                        WHEN RegionUnidad like '%Coquimbo%' THEN 4000
                        WHEN RegionUnidad like '%Valparaíso%' THEN 5000
                        WHEN RegionUnidad like '%Valparaiso%' THEN 5000
                        WHEN RegionUnidad like '%Higgins%' THEN 6000
                        WHEN RegionUnidad like '%Maule%' THEN 7000
                        WHEN RegionUnidad like '%Bio%' THEN 8000
                        WHEN RegionUnidad like '%Araucan%' THEN 9000
                        WHEN RegionUnidad like '%Lagos%' THEN 10000
                        WHEN RegionUnidad like '%Aysén%' THEN 11000
                        WHEN RegionUnidad like '%Aysen%' THEN 11000
                        WHEN RegionUnidad like '%Magallanes%' THEN 12000
                        WHEN RegionUnidad like '%Metropolitana%' THEN 13000
                        WHEN RegionUnidad like '%Ríos%' THEN 14000
                        WHEN RegionUnidad like '%Rios%' THEN 14000
                        WHEN RegionUnidad like '%Arica%' THEN 15000
                        WHEN RegionUnidad like '%Ñuble%' THEN 16000
                        WHEN RegionUnidad like '%Nuble%' THEN 16000
       ...
       l.MontoLineaAdjudica * cm.VMEUR as monto_adjudicado_eur,
       l.MontoLineaAdjudica * cm.VMUTM as monto_adjudicado_utm,
       l.MontoLineaAdjudica * cm.VMCLF as monto_adjudicado_uf,
       ROW_NUMBER() OVER (PARTITION BY fechaAdjudicacion, id_rubro, codigoProductoONU, codigoUnidad, codigoProveedor ORDER BY fechaAdjudicacion) AS NumeroDeFila
  FROM mercadopublico.dbo.licitaciones l
  LEFT JOIN mp_estrella_db.dbo.dim_rubro r
